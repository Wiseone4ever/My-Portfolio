...
>>>>>>> ada831676c3d57dd169bd906b3f4742b74da0632
```{r}
library(readr)
library(RSQLite)
library(tibble) 
...
```{r}
ingest_orders <- function(df, my_connection) {
...
ingest_orders(orders_df, my_connection)
...
ingest_gift_card_data <- function(df, my_connection) {
...
my_connection <- RSQLite::dbConnect(RSQLite::SQLite(), "database/ecommerce_database_v1.db")
ingest_gift_card_data(gift_card_df, my_connection)
```
...
```{r}
#dbExecute(my_connection,"DELETE FROM GIFT_CARD;")
```

SHIPMENTS

```{r}
ingest_shipment_data <- function(df, my_connection) {
 ...
 df$dispatch_timestamp <- ifelse(is.na(df$dispatch_timestamp), NA, as.Date(df$dispatch_timestamp))
  df$delivered_timestamp <- ifelse(is.na(df$delivered_timestamp), NA, as.Date(df$delivered_timestamp))
...
my_connection <- RSQLite::dbConnect(RSQLite::SQLite(), "database/ecommerce_database_v1.db")
ingest_shipment_data(shipment_df, my_connection)
```
...
```{r}
#dbExecute(my_connection,"DELETE FROM SHIPMENT;")
```
