...
ingest_orders <- function(df, my_connection) {
  ...
  required_columns <- c("order_id", "order_status", "quantity", "amount")
  df <- df[!rowSums(is.na(df[required_columns])) > 0, ]
  ...
  for(i in 1:nrow(df)) {
    ...
    # Data validation for quantity and amount
    ...
    insert_query <- sprintf("INSERT INTO ORDERS (order_id, customer_id, product_id, shipment_id, gift_card_id, payment_method, quantity, order_timestamp, payment_timestamp, order_status) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', %d, '%s', '%s', '%s')",
                            df$order_id[i], df$customer_id[i], df$product_id[i], df$shipment_id[i], df$gift_card_id[i], df$payment_method[i], df$quantity[i], df$order_timestamp[i], df$payment_timestamp[i], df$order_status[i], df$amount[i])
    tryCatch({
      dbExecute(my_connection, insert_query)
      cat(sprintf("Successfully inserted row: %d\n", i))
    }, error = function(e) {
