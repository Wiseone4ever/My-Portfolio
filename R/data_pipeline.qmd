  ...
  shipment_df <- shipment_df %>%
    mutate(
      ...
      delivered_timestamp = if_else(status == "Delivered"
                              , dispatch_timestamp + days(sample(1:14, 1))
                              , delivered_timestamp + days(sample(1:14, 1)))
    )
...
top_products_df <- dbGetQuery(my_connection, query_3)
...
# Plot using ggplot2
ggplot(top_products_df, aes(x = reorder(product_name, total_amount), y = total_amount, fill = cat_name)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  coord_flip() +
  labs(title = "Top 3 Products by Total Amount",
       x = "Product Name",
...
```{r}
# Execute the query and store the result in a data frame
cancelled_df <- dbGetQuery(my_connection, query_6)
